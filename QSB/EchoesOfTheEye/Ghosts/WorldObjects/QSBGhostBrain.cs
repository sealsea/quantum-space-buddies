...
public class QSBGhostBrain : WorldObject<GhostBrain>, IGhostObject
{
	...
	private void DataLines(QSBGhostData data, GhostController controller)
	{
		...
		{
			if (data.timeSincePlayerLocationKnown[player] != float.PositiveInfinity)
			{
				Popcron.Gizmos.Line(controller.transform.position, controller.LocalToWorldPosition(data.lastKnownPlayerLocation[player].localPosition), Color.magenta);
				Popcron.Gizmos.Sphere(controller.LocalToWorldPosition(data.lastKnownPlayerLocation[player].localPosition), 1f, Color.magenta);
			}
		}
	}
	...
	public bool CheckDreadAudioConditions()
	{
		return _currentAction != null
			&& _data.playerLocation[QSBPlayerManager.LocalPlayer].distance < 10f
	...
	public bool HearCallForHelp(PlayerInfo player, Vector3 playerLocalPosition, float reactDelay)
	{
		if (_data.isAlive && !_data.hasWokenUp)
		{
			return false;
		...
		_data.lastKnownPlayerLocation[player].UpdateLocalPosition(playerLocalPosition, AttachedObject._controller);
		_data.wasPlayerLocationKnown[player] = true;
		_data.timeSincePlayerLocationKnown[player] = 0f;
		return true;
	}

	public void HintPlayerLocation(PlayerInfo player)
	{
		HintPlayerLocation(_data.playerLocation[player].localPosition, Time.time, player);
	}

	public void HintPlayerLocation(Vector3 localPosition, float informationTime, PlayerInfo player)
	{
		if (!_data.hasWokenUp || _data.isPlayerLocationKnown[player])
		{
			return;
		}
		...
		{
			_data.lastKnownPlayerLocation[player].UpdateLocalPosition(localPosition, AttachedObject._controller);
			_data.wasPlayerLocationKnown[player] = true;
			_data.timeSincePlayerLocationKnown[player] = 0f;
		}
	}
	...
	public void FixedUpdate()
	{
		...
		_data.FixedUpdate_Data(AttachedObject._controller);
	...
	public void FixedUpdate_ThreatAwareness()
	{
		...
		if (!AttachedObject._intruderConfirmPending
			&& (_data.threatAwareness > GhostData.ThreatAwareness.EverythingIsNormal || _data.playerLocation.Any(x => x.Value.distance < 20f) || _data.sensors.Any(x => x.Value.isPlayerIlluminatedByUs))
			&& (_data.sensors.Any(x => x.Value.isPlayerVisible) || _data.sensors.Any(x => x.Value.inContactWithPlayer)))
		{
			AttachedObject._intruderConfirmedBySelf = true;
			AttachedObject._intruderConfirmPending = true;
			var num = Mathf.Lerp(0.1f, 1.5f, Mathf.InverseLerp(5f, 25f, _data.playerLocation.Select(x => x.Value.distance).Min()));
	...
	public void OnCallForHelp()
	{
		...
		if (AttachedObject._helperGhosts != null)
		{
			...
			{
				AttachedObject._helperGhosts[i].GetWorldObject<QSBGhostBrain>().HearCallForHelp(_data.InterestedPlayer, _data.playerLocation[_data.InterestedPlayer].localPosition, 3f);
			}
		}
	}
