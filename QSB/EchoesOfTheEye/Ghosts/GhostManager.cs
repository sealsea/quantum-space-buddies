...
internal class GhostManager : WorldObjectManager
{
	...
	public static void CustomOnHotelDepthsGhostsIdentifiedIntruder(GhostBrain ghostBrain, QSBGhostData ghostData)
	{
		...
		{
			if (!(_hotelDirector._hotelDepthsGhosts[i] == ghostBrain) && _hotelDirector._hotelDepthsGhosts[i].HearGhostCall(ghostData.playerLocation[ghostData.InterestedPlayer].localPosition, num, false))
			{
	...
	public static void CustomOnGhostIdentifyIntruder(GhostBrain ghostBrain, QSBGhostData ghostData)
	{
		...
		for (int i = 0; i < _partyPathDirector._directedGhosts.Length; i++)
		{
			if (!(_partyPathDirector._directedGhosts[i] == ghostBrain))
			{
				...
				if (flag && num2 < 50f && _partyPathDirector._directedGhosts[i].HearGhostCall(ghostData.playerLocation[ghostData.InterestedPlayer].localPosition, num, true))
				{
					_partyPathDirector._directedGhosts[i].GetWorldObject<QSBGhostBrain>().HintPlayerLocation(ghostData.InterestedPlayer);
					num += Random.Range(2f, 3f);
				}
			}
		}
	...
	public static void CustomOnCityGhostsIdentifiedIntruder(GhostBrain ghostBrain, QSBGhostData ghostData)
	{
		...
		for (int i = 0; i < _zone2Director._cityGhosts.Length; i++)
		{
			if (!(_zone2Director._cityGhosts[i] == ghostBrain) && _zone2Director._cityGhosts[i].HearGhostCall(ghostData.playerLocation[ghostData.InterestedPlayer].localPosition, num, false))
			{
				num += Random.Range(2f, 3f);
			}
		}
